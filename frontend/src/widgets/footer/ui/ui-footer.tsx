...
import { footerDescriptions } from '../model/config'
	...
	return (
		<footer className='flex items-center bg-[#17171BFF] px-3 py-10 text-white'>
			<div className='flex flex-col  mt-6 ml-10 items-center'>
				...
				<div className={'flex justify-center flex-row items-start pt-4'}>
			...
			<div className='gap-y-12 sm:gap-x-6 lg:gap-x-8 lg:gap-y-0 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 ml-8'>
				{footerDescriptions.map(perk => (
					<div key={perk.id} className='flex flex-col items-start mt-6 ml-10 text-center'>
						<h3 key={perk.id} className='mr-4 text-[#818181FF] text-bold text-xl'>
							{perk.name}
						</h3>
